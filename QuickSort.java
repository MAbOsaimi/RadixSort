public class QuickSort {
    ...
    private static int partition(String[] arr, int low, int high, int place) {
        String pivot = arr[high];
        char pivotChar = ' ', currChar = ' ';

        if (place < pivot.length()) {
            pivotChar = pivot.toLowerCase().charAt(place);
        }
        int i = low - 1;
        for (int j = low; j < high; j++) {
            if (place < arr[j].length()) {
                currChar = arr[j].toLowerCase().charAt(place);
            }
